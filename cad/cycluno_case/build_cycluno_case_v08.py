...
doc = FreeCAD.newDocument("CyclUnoCase_v08")

# ============================================================================
# v0.8 — FINAL ergonomic Palm Deck. Dimensions reconciled against the REAL
...
base_obj = doc.addObject("Part::Feature", "Base_v08")
base_obj.Shape = base
...
for (hx, hy) in UNO_HOLES:
    ...
    lid = lid.cut(h)
lid_obj = doc.addObject("Part::Feature", "Lid_v08")
lid_obj.Shape = lid

doc.recompute()
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CyclUnoCase_v08.FCStd")
doc.saveAs(out)
print("BASE v08 V=", round(base.Volume/1000, 1), "cm3  z[0,%.0f]" % H, "watertight=", base.isValid())
print("LID  v08 V=", round(lid.Volume/1000, 1), "cm3  valid=", lid.isValid())
print("saved:", out)
